...
void resume_aux(Media c, Descriptor descriptor, Document d, media_tag)
{
  ...
  gntl::algorithm::structure::media::resume (c, descriptor, d);
}

template <typename Context, typename Descriptor, typename Document>
void resume_aux(Context c, Descriptor descriptor, Document d, context_tag)
{
  typedef typename boost::unwrap_reference<Context>::type component_type;
  typedef typename boost::unwrap_reference<Document>::type document_type;

  gntl::algorithm::structure::context::resume (c, descriptor, d);
}

template <typename Switch, typename Descriptor, typename Document>
void resume_aux(Switch s, Descriptor descriptor, Document d, switch_tag)
{
...
void resume_aux(Port p, Descriptor descriptor, Document d, port_tag)
{
  //algorithm::structure::port::resume(p, d);
}

template <typename Component, typename Descriptor, typename Document>
void resume(Component c, Descriptor descriptor, Document d)
{
  ...
  component::resume_aux(c, descriptor, d, tag_type());
}

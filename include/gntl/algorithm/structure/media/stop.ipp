...
#ifndef GNTL_ALGORITHM_STRUCTURE_MEDIA_STOP_IPP
#define GNTL_ALGORITHM_STRUCTURE_MEDIA_STOP_IPP
...
#define BOOST_PP_ITERATION_PARAMS_1 (3, (0, BOOST_PP_DEC (GNTL_MAX_ARGS), "gntl/algorithm/structure/media/stop.ipp"))
...
#endif
#else
template <typename StopMediaTraits, typename Media, typename Descriptor
          , typename Document
  ...
  )
{
    ...
    GNTL_DEBUG_LOG("stopping media no refer " << concept::identifier(m) << std::endl)
  ...
  )
{
  ...
  typedef typename media_traits::presentation_type presentation_type;
  typedef concept::structure::presentation_traits<presentation_type> presentation_traits;
  GNTL_DEBUG_LOG("stopping media" << std::endl)
  ...
  if(concept::has_refer(m))
  {
    ...
    if(media_lookup != lookupable_traits::not_found(lookupable))
    {
      if(!concept::has_instance(m) || concept::instance(m) == "new")
      {
        ...
        media::stop_no_refer<StopMediaTraits>(new_media, descriptor, document
  ...
  {
    media::stop_no_refer<StopMediaTraits>(m, descriptor, document
                                          BOOST_PP_ENUM_TRAILING_PARAMS(BOOST_PP_ITERATION(), a));
  }
}
#endif

...
#define BOOST_TEST_MODULE parse_media_context1
...
BOOST_AUTO_TEST_CASE (parse_media_context)
{
    ...
    ::xmlInitParser ();
    {
        ...
        context_range crange
          = context_traits::context_all (context);

        GNTL_GLOBAL_ALLOC_CHECK_EQUAL (gntl::range::distance (crange), 0u);

        context_traits::media_range medias = context_traits::media_all (context);

        GNTL_GLOBAL_ALLOC_REQUIRE (boost::distance(medias) > 0);

        GNTL_GLOBAL_ALLOC_CHECK_EQUAL (media_traits::identifier (*boost::begin(medias)), "media1");
        GNTL_GLOBAL_ALLOC_CHECK (!media_traits::has_source (*boost::begin(medias)));
        GNTL_GLOBAL_ALLOC_CHECK (!media_traits::has_refer (*boost::begin(medias)));
        GNTL_GLOBAL_ALLOC_CHECK (!media_traits::has_instance (*boost::begin(medias)));
        GNTL_GLOBAL_ALLOC_CHECK (!media_traits::has_type (*boost::begin(medias)));
        GNTL_GLOBAL_ALLOC_CHECK (!media_traits::has_descriptor (*boost::begin(medias)));
    }
    ::xmlCleanupParser ();

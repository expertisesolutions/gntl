...
#define BOOST_TEST_MODULE parse_binds_link_context1
...
BOOST_AUTO_TEST_CASE (parse_binds_link_context1)
{
    ...
    ::xmlInitParser ();
    {
        ...
        typedef concept::parser::link_traits<parser::libxml2::dom::link> link_traits;
        ...
        GNTL_GLOBAL_ALLOC_CHECK_EQUAL (boost::distance (crange), 0u);

        context_traits::link_range link_range = context_traits::link_all (context);

        GNTL_GLOBAL_ALLOC_REQUIRE (boost::distance(link_range) > 0);

        GNTL_GLOBAL_ALLOC_CHECK_EQUAL (link_traits::identifier
                                       (*boost::begin(link_range)), "link1");
        GNTL_GLOBAL_ALLOC_CHECK_EQUAL (link_traits::xconnector
                                       (*boost::begin(link_range)), "connector1");

        link_traits::bind_range bind_range = link_traits::bind(*boost::begin(link_range));

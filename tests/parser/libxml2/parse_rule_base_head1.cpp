...
#define BOOST_TEST_MODULE parse_rule_base_head1
...
BOOST_AUTO_TEST_CASE (parse_rule_base_head)
{
    ...
    ::xmlInitParser ();
    {
        ...
        GNTL_GLOBAL_ALLOC_REQUIRE (head_traits::has_rule_base (head));

        struct parser::libxml2::dom::rule_base  base = head_traits::rule_base (head);

        typedef concept::parser::rule_base_traits<parser::libxml2::dom::rule_base> rule_base_traits;

        GNTL_GLOBAL_ALLOC_CHECK (!rule_base_traits::has_identifier (base));
        GNTL_GLOBAL_ALLOC_CHECK (boost::empty(rule_base_traits::rule_all (base)));

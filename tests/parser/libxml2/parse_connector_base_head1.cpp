...
#define BOOST_TEST_MODULE parse_connector_base_head1
...
BOOST_AUTO_TEST_CASE (parse_connector_base_head)
{
    ...
    ::xmlInitParser ();
    {
        ...
        GNTL_GLOBAL_ALLOC_REQUIRE (head_traits::has_connector_base (head));
        
        parser::libxml2::dom::connector_base 
          base = head_traits::connector_base (head);

        typedef concept::parser::connector_base_traits
          <parser::libxml2::dom::connector_base>
          connector_base_traits;

        GNTL_GLOBAL_ALLOC_CHECK (boost::empty(connector_base_traits
                                              ::causal_connector_all(base)));
    }
    ::xmlCleanupParser ();

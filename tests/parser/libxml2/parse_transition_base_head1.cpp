...
#define BOOST_TEST_MODULE parse_transition_base_head1
...
BOOST_AUTO_TEST_CASE (parse_transition_base_head)
{
    ...
    ::xmlInitParser ();
    {
        ...
        BOOST_REQUIRE_EQUAL (document.root ().name (), "head");
        ...
        GNTL_GLOBAL_ALLOC_REQUIRE (head_traits::has_transition_base (head));

        parser::libxml2::dom::transition_base base = head_traits::transition_base (head);

        typedef concept::parser::transition_base_traits<parser::libxml2::dom::transition_base>
          transition_base_traits;

        GNTL_GLOBAL_ALLOC_CHECK (!transition_base_traits::has_identifier (base));
        GNTL_GLOBAL_ALLOC_CHECK (gntl::range::empty(transition_base_traits::transition_all(base)));

        GNTL_GLOBAL_ALLOC_CHECK (gntl::range::empty(head_traits::region_base_all (head)));

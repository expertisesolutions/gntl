...
#define BOOST_TEST_MODULE parse_empty_context1
...
BOOST_AUTO_TEST_CASE (parse_empty_context)
{
    ...
    ::xmlInitParser ();
    {
        ::xmlDocPtr xmldoc = xmlParseDoc(static_cast<const xmlChar*>(static_cast<const void*>(empty_context)));
        ...
        context_range range = context_traits::context_all (context);

        GNTL_GLOBAL_ALLOC_CHECK_EQUAL (gntl::range::distance(range), 0u);
    }
    ::xmlCleanupParser ();

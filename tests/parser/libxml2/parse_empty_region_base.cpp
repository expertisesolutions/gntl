...
#define BOOST_TEST_MODULE parse_empty_region_base
...
const char empty_region_base[]
= "<regionBase />";

namespace parser = gntl::parser;
namespace concept = gntl::concept;

BOOST_AUTO_TEST_CASE (parse_empty_region_base)
{
    ...
    ::xmlInitParser ();
    {
        ::xmlDocPtr xmldoc = xmlParseDoc(static_cast<const xmlChar*>(static_cast<const void*>(empty_region_base)));
        ...
        GNTL_GLOBAL_ALLOC_CHECK_EQUAL (boost::distance(region_base_traits::region_all (region_base)), 0u);
    }
    ::xmlCleanupParser ();

...
#define BOOST_TEST_MODULE abort_media_normal_action_default_descriptor1
#include <boost/test/included/unit_test.hpp>

#include <gntl/algorithm/structure/media/abort.hpp>
...
bool executor_abort_run = false
    , executor_abort_async_run = false;
...
struct executor_traits< ::executor>
{
  ...
  {
    std::cout << "executor_abort" << std::endl;
    executor_abort_run = true;
  }
...
struct abort_traits
{
  ...
  static void abort(Media m, Presentation p, Document)
  {
    ...
    presentation_traits::abort(p);
  }

  template <typename Media, typename Presentation, typename Handler>
  static void async_abort(Media m, Presentation p, Handler h)
  {
    abort(m, p);
  }
};
...
BOOST_AUTO_TEST_CASE (abort_media_normal_action_default_descriptor1)
{
    ...
    presentation_type presentation = media_traits::prepare_presentation
      (media, descriptor_type());
    ...
    algorithm::structure::media::abort<abort_traits> (boost::ref (media), descriptor_type(), gntl::ref(document));
    ...
    // algorithm::structure::media::async_abort<abort_traits> (boost::ref (media)
    //                                , &::handler);

    BOOST_CHECK(presentation_traits::is_sleeping(presentation));

    BOOST_CHECK_EQUAL (::executor_abort_run, true);
    BOOST_CHECK_EQUAL (::executor_abort_async_run, true);
}

...
#define BOOST_TEST_MODULE stop_media_user_defined_action_default_descriptor1
...
struct stop_traits
{
  ...
  static void stop(Media m, Presentation p, Document document)
  {
...
BOOST_AUTO_TEST_CASE (stop_media_user_defined_action_default_descriptor1)
{
    ...
    algorithm::structure::media::stop<stop_traits> (gntl::ref (media), descriptor_type(), gntl::ref(document));

    BOOST_CHECK(presentation_traits::is_sleeping(presentation));
    presentation_traits::start(presentation);
    BOOST_CHECK(presentation_traits::is_occurring(presentation));

    // algorithm::structure::media::async_stop<stop_traits> (boost::ref (media), &::handler);
    BOOST_CHECK(presentation_traits::is_sleeping(presentation));

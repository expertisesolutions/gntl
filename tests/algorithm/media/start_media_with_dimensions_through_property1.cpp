...
#define BOOST_TEST_MODULE start_media_with_dimensions_through_property1
...
const char xml_media[]
  = "<media id=\"identifier\" src=\"source1\">\n"
    "  <property name=\"region\" value=\"region1\" />\n"
    "</media>"
 ;
 ;
...
BOOST_AUTO_TEST_CASE (start_media_with_dimensions_through_property1)
{
    ...
    algorithm::structure::media::start (boost::ref (media), interface_
                                        , boost::none // descriptor
                                        , boost::ref (document), full);

    BOOST_REQUIRE(media_traits::has_presentation(media, descriptor_type()));
    typedef gntl::concept::structure::presentation_traits<presentation_type> presentation_traits;
    presentation_type presentation = media_traits::get_presentation(media, boost::none);

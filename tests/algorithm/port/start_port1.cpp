...
#define BOOST_TEST_MODULE start_port1
#include <boost/test/included/unit_test.hpp>

#include <gntl/algorithm/structure/media.hpp>
#include <gntl/algorithm/structure/port/start_normal_action.hpp>
...
struct executor_traits< ::executor>
{
                     ...
                     , gntl::algorithm::structure::media::dimensions dim)
  {
    ...
    algorithm::structure::media::dimensions d
      = {0, 0, 1280, 720, 0};
...
BOOST_AUTO_TEST_CASE (start_port1)
{
    ...
    BOOST_REQUIRE (!boost::empty(context_traits::port_all (context)));
    dom::port port = *boost::begin (context_traits::port_all (context));
    ...
    algorithm::structure::port::start (gntl::ref(port), gntl::ref(context), gntl::ref(document), full);
